...
describe('getCurrentCpuTicks()', () => {
    test('calculates cpu load based on os.cpus', () => {
        ...
        const cpusMock = vitest.spyOn(os, 'cpus').mockReturnValue([
...
describe('getCpuInfo()', () => {
  ...
  test('returns bare metal cpu ticks when not containerised', async () => {
    // Simulate non-AWS Lambda and non-containerised environment.
  ...
  test('returns container-aware cpu usage when containerised with quota set', async () => {
  ...
  test('returns bare metal cpu ticks when containerised but no cgroup quota', async () => {
    isContainerisedSpy.mockResolvedValueOnce(true);
    getCgroupsVersionSpy.mockResolvedValueOnce("V1");
    ...
    const cpusMock = vitest.spyOn(os, 'cpus').mockReturnValue([
      { times: { user: 300, nice: 0, sys: 150, idle: 150, irq: 0 } },
    ] as os.CpuInfo[]);
    const result = await getCpuInfo();
